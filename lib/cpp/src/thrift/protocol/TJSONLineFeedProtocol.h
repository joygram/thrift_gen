...
#ifndef _THRIFT_PROTOCOL_TJSONLINEFEEDPROTOCOL_H_
#define _THRIFT_PROTOCOL_TJSONLINEFEEDPROTOCOL_H_ 1
 ...
 *
 */
class TJSONLineFeedProtocol : public TVirtualProtocol<TJSONLineFeedProtocol> {
public:
  TJSONLineFeedProtocol(boost::shared_ptr<TTransport> ptrans);

  ~TJSONLineFeedProtocol();

private:
  void pushContext(boost::shared_ptr<TJSONLineFeedContext> c);
...
public:
  ...
  using TVirtualProtocol<TJSONLineFeedProtocol>::readBool;
...
private:
  ...
  std::stack<boost::shared_ptr<TJSONLineFeedContext> > contexts_;
  boost::shared_ptr<TJSONLineFeedContext> context_;
 ...
 */
class TJSONLineFeedProtocolFactory : public TProtocolFactory {
public:
  TJSONLineFeedProtocolFactory() {}

  virtual ~TJSONLineFeedProtocolFactory() {}

  boost::shared_ptr<TProtocol> getProtocol(boost::shared_ptr<TTransport> trans) {
    return boost::shared_ptr<TProtocol>(new TJSONLineFeedProtocol(trans));
  }
};
}
}
...
std::string ThriftJSONLineFeedString(const ThriftStruct& ts) {
  using namespace apache::thrift::transport;
  using namespace apache::thrift::protocol;
  TMemoryBuffer* buffer = new TMemoryBuffer;
  boost::shared_ptr<TTransport> trans(buffer);
  TJSONLineFeedProtocol protocol(trans);
...
#endif // #define _THRIFT_PROTOCOL_TJSONLINEFEEDPROTOCOL_H_ 1
