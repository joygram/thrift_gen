...
#if BOOST_VERSION >= 107000 // by joygram for boost 1.70 support
#  include <boost/predef/other/endian.h>
#else
#  include <boost/detail/endian.hpp>
#endif // by joygram for boost 1.70 support
...
public:
  ...
  //{ support unsigned by joygram 2016/07/29
  uint32_t readByte(uint8_t& ubyte) {
	  T_VIRTUAL_CALL();
	  int8_t& byte = (int8_t&)ubyte;
	  return readByte_virt(byte);
  }

  uint32_t readI16(uint16_t& ui16) {
	  T_VIRTUAL_CALL();
	  int16_t& i16 = (int16_t&)ui16;
	  return readI16_virt(i16);
  }

  uint32_t readI32(uint32_t& ui32) {
	  T_VIRTUAL_CALL();
	  int32_t& i32 = (int32_t&)ui32;
	  return readI32_virt(i32);
  }

  uint32_t readI64(uint64_t& ui64) {
	  T_VIRTUAL_CALL();
	  int64_t& i64 = (int64_t&)ui64;
	  return readI64_virt(i64);
  }
  //}
