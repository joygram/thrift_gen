...
public:
	...
	*/
	...
	void generate_field(t_field * field, int32_t order);
...
private:
	...
	void start_empty_object();
	void start_object();
	void start_array();
	void write_key(string key, string val);
	void write_int_key(int key, string val);
	void write_key_int(string key, int val);
	void write_key_only(string key); // for enum name 
	void write_value_int(int val); // for add index

	void end_empty_object();
...
string t_gen_json_generator::escapeJsonString(const string& input) {
...
void t_gen_json_generator::start_empty_object(){
	_commaNeeded.push(false);
}

void t_gen_json_generator::start_object(){
	f_json_ << "{";
	_commaNeeded.push(false);
}
...
void t_gen_json_generator::write_int_key(int key, string val)
{
...
void t_gen_json_generator::write_key(string key, string val){
	write_comma_if_needed();
	f_json_ << quot << key << quot << ":" << quot << escapeJsonString(val) << quot;
	indicate_comma_needed();
}

void t_gen_json_generator::write_key_int(string key, int val){
	write_comma_if_needed();
	f_json_ << quot << key << quot << ":" << quot << val << quot;
	indicate_comma_needed();
}

void t_gen_json_generator::write_value_int(int val)
{
	f_json_ << quot << val << quot << ":";
}

void t_gen_json_generator::write_key_only(string key)
{
	f_json_ << quot << key << quot << ":" ;
}


void t_gen_json_generator::end_empty_object()
{
	_commaNeeded.pop();
}

void t_gen_json_generator::end_object(bool newLine){
	f_json_ << "}";
	if (newLine) f_json_ << endl;
	_commaNeeded.pop();
}

void t_gen_json_generator::end_array(bool newLine){
	f_json_ << "]";
	if (newLine) f_json_ << endl;
	_commaNeeded.pop();
}
...
void t_gen_json_generator::generate_consts(vector<t_const*> consts){
	vector<t_const*>::iterator c_iter;
	f_json_ << ",\"constants\":";
	start_array();
	f_json_ << endl;
...
//	write_comma_if_needed();
	...
	string enumName = ns + tenum->get_name();
	write_key_only(enumName);
	...
	{
		t_enum_value* val = (*val_iter);
		write_int_key(val->get_value(), val->get_name());
	}
	end_object(true);
...
//
//	//start_array();
//	start_object(); // for index to key 
//	for (; mem_iter != members.end(); mem_iter++)
//	{
//		generate_field((*mem_iter));
//	}
//	end_object(false); // for index to key
//	//end_array(false);
//
//	end_object(true);
//}
...
void t_gen_json_generator::generate_function(t_function* tfunc)
{
	...
	for (; ex_iter != excepts.end(); ex_iter++)
	{
		generate_field((*ex_iter), ++order_number);
	}
	end_object(false);
}
...
void t_gen_json_generator::generate_field(t_field * field, int32_t order)
{
	...
	write_value_int(field->get_key());

	start_object();
	write_key("name", field->get_name());
	write_key("type", get_type_name(field->get_type()));
	write_key("datatype", get_datatype_name(field->get_type())); // by joygram 2015/06/24
	write_key("typedef", get_typedef_name(field->get_type())); // by joygram 2015/06/24
	...
	// add thrift index (key) by joygram 2016/03/21;
	{
		...
		out << field->get_key();
		write_key("thrift_index", out.str());
	}
...
string t_gen_json_generator::get_const_value(t_const_value* tvalue){
...
THRIFT_REGISTER_GENERATOR(gen_json, "GEN_JSON, for GEN meta data", "")
