 ...
 *
 */
class t_gen_cpp_generator : public t_oop_generator {
public:
  t_gen_cpp_generator(t_program* program,
...
private:
  ...
  // The ProcessorGenerator_gen is used to generate parts of the code,
  ...
  // themselves.
  friend class ProcessorGenerator_gen;
};
 ...
 * streams.
 */
void t_gen_cpp_generator::init_generator() {
  ...
  f_types_ << "#include <thrift/stdcxx.h>" << endl; // for use boost::function by jogyram 2022/06/06
  f_types_ << "#include <memory>" << endl;

  // Include other Thrift includes
  const vector<t_program*>& includes = program_->get_includes();
  for (auto include : includes) {
    f_types_ << "#include \"" << get_include_prefix(*include) << include->get_name() << "_types.h\""
             << endl;
 ...
 */
void t_gen_cpp_generator::close_generator() {
  // Close namespace
 ...
 */
void t_gen_cpp_generator::generate_typedef(t_typedef* ttypedef) {
...
void t_gen_cpp_generator::generate_enum_constant_list(std::ostream& f,
                                                      const vector<t_enum_value*>& constants,
                                                      const char* prefix,
                                                      const char* suffix,
                                                      bool include_values) {
 ...
 */
void t_gen_cpp_generator::generate_enum(t_enum* tenum) {
  ...
  */
  ...
  f_types_impl_ << indent() << "const std::map<int, const char*> _" << tenum->get_name()
                ...
                << "::apache::thrift::TEnumIterator(-1, nullptr, nullptr));" << endl
                << endl;
...
void t_gen_cpp_generator::generate_enum_ostream_operator_decl(std::ostream& out, t_enum* tenum) {
...
void t_gen_cpp_generator::generate_enum_ostream_operator(std::ostream& out, t_enum* tenum) {

  // If we've been told the consuming application will provide an ostream
  // operator definition then we only make a declaration:i
...
void t_gen_cpp_generator::generate_enum_to_string_helper_function_decl(std::ostream& out,
                                                                       t_enum* tenum) {
...
void t_gen_cpp_generator::generate_enum_to_string_helper_function(std::ostream& out,
                                                                  t_enum* tenum) {
  if (!has_custom_ostream(tenum)) {
    out << "std::string to_string(const ";
    if (gen_pure_enums_) {
 ...
 * Generates a class that holds all the constants.
    enum to consts by joygram
 */
void t_gen_cpp_generator::generate_consts(std::vector<t_const*> consts) {
  /*
    ...
    // Start ifndef
      f_consts << "#ifndef " << program_name_ << "_CONSTANTS_H" << endl << "#define " <<
    program_name_
               << "_CONSTANTS_H" << endl << endl << "#include \"" <<
  ...
  */
  ...
  ostream& f_consts = f_types_;           // enum to consts by joygram
  ostream& f_consts_impl = f_types_impl_; // enum to consts by joygram
  ...
  f_consts << "class "
           ...
           << "  "
           << "Constants();" << endl
           << endl; // by joygram : program_name_ removed
  indent_up();
  ...
  */
  f_consts_impl << "const "
                ...
                << endl
                << "Constants::"
                << "Constants() {" << endl; // change const name by joygram
  indent_up();
  for (c_iter = consts.begin(); c_iter != consts.end(); ++c_iter) {
  ...
  */
  f_consts << endl
           << "extern const "
           << "Constants constants;" << endl
           << endl; // remove program name by joygram
}
 ...
 */
void t_gen_cpp_generator::print_const_value(ostream& out,
                                            string name,
                                            t_type* type,
                                            t_const_value* value) {
 ...
 *
 */
string t_gen_cpp_generator::render_const_value(ostream& out,
                                               string name,
                                               t_type* type,
                                               t_const_value* value) {
  (void)name;
...
void t_gen_cpp_generator::generate_forward_declaration(t_struct* tstruct) {
 ...
 */
void t_gen_cpp_generator::generate_cpp_struct(t_struct* tstruct, bool is_exception) {
...
void t_gen_cpp_generator::generate_copy_constructor(ostream& out,
                                                    t_struct* tstruct,
                                                    bool is_exception) {
  generate_constructor_helper(out, tstruct, is_exception, /*is_move=*/false);
}

void t_gen_cpp_generator::generate_move_constructor(ostream& out,
                                                    t_struct* tstruct,
                                                    bool is_exception) {
...
void t_gen_cpp_generator::generate_constructor_helper(ostream& out,
                                                      t_struct* tstruct,
                                                      bool is_exception,
                                                      bool is_move) {
...
void t_gen_cpp_generator::generate_assignment_operator(ostream& out, t_struct* tstruct) {
  generate_assignment_helper(out, tstruct, /*is_move=*/false);
}

void t_gen_cpp_generator::generate_move_assignment_operator(ostream& out, t_struct* tstruct) {
  generate_assignment_helper(out, tstruct, /*is_move=*/true);
}

void t_gen_cpp_generator::generate_assignment_helper(ostream& out,
                                                     t_struct* tstruct,
                                                     bool is_move) {
  std::string tmp_name = tmp("other");
 ...
 */
void t_gen_cpp_generator::generate_struct_declaration(ostream& out,
                                                      t_struct* tstruct,
                                                      bool is_exception,
                                                      bool pointers,
                                                      bool read,
...
void t_gen_cpp_generator::generate_struct_definition(ostream& out,
 ...
 * @param tstruct The struct
 */
void t_gen_cpp_generator::generate_struct_reader(ostream& out, t_struct* tstruct, bool pointers) {
  if (gen_templates_) {
    out << indent() << "template <class Protocol_>" << endl
 ...
 * @param tstruct The struct
 */
void t_gen_cpp_generator::generate_struct_writer(ostream& out, t_struct* tstruct, bool pointers) {
 ...
 */
void t_gen_cpp_generator::generate_struct_result_writer(ostream& out,
                                                        t_struct* tstruct,
                                                        bool pointers) {
  string name = tstruct->get_name();
  const vector<t_field*>& fields = tstruct->get_sorted_members();
 ...
 * @param tstruct The struct
 */
void t_gen_cpp_generator::generate_struct_swap(ostream& out, t_struct* tstruct) {
  if (tstruct->get_name() == "a" || tstruct->get_name() == "b") {
    out << indent() << "void swap(" << tstruct->get_name() << " &a1, " << tstruct->get_name()
        << " &a2) {" << endl;
  } else {
...
void t_gen_cpp_generator::generate_struct_ostream_operator_decl(std::ostream& out,
                                                                t_struct* tstruct) {
...
void t_gen_cpp_generator::generate_struct_ostream_operator(std::ostream& out, t_struct* tstruct) {
  if (!has_custom_ostream(tstruct)) {
...
void t_gen_cpp_generator::generate_struct_print_method_decl(std::ostream& out, t_struct* tstruct) {
  out << "void ";
  if (tstruct) {
    out << tstruct->get_name() << "::";
  }
...
void t_gen_cpp_generator::generate_exception_what_method_decl(std::ostream& out,
                                                              t_struct* tstruct,
                                                              bool external) {
  out << "const char* ";
  if (external) {
...
namespace struct_ostream_operator_generator_gen {
 ...
 * Generates operator<<
 */
void t_gen_cpp_generator::generate_struct_print_method(std::ostream& out, t_struct* tstruct) {
  out << indent();
  ...
  struct_ostream_operator_generator_gen::generate_fields(out, tstruct->get_members(), indent());
  out << indent() << "out << \")\";" << endl;
 ...
 */
void t_gen_cpp_generator::generate_exception_what_method(std::ostream& out, t_struct* tstruct) {
  out << indent();
 ...
 */
void t_gen_cpp_generator::generate_service(t_service* tservice) {
 ...
 */
void t_gen_cpp_generator::generate_service_helpers(t_service* tservice) {
 ...
 * @param tservice The service to generate a header definition for
 */
void t_gen_cpp_generator::generate_service_interface(t_service* tservice, string style) {
 ...
 */
void t_gen_cpp_generator::generate_service_interface_factory(t_service* tservice, string style) {
  string service_if_name = service_name_ + style + "If";
 ...
 * @param tservice The service to generate a header definition for
 */
void t_gen_cpp_generator::generate_service_null(t_service* tservice, string style) {
  string extends = "";
  if (tservice->get_extends() != nullptr) {
...
void t_gen_cpp_generator::generate_function_call(ostream& out,
                                                 t_function* tfunction,
                                                 string target,
                                                 string iface,
                                                 string arg_prefix) {
...
void t_gen_cpp_generator::generate_service_async_skeleton(t_service* tservice) {
  string svcname = tservice->get_name();
  ...
  f_skeleton << indent() << svcname << "AsyncHandler() {" << endl
             << indent() << "  syncHandler_ = std::unique_ptr<" << svcname << "Handler>(new "
  ...
  indent_up();
  f_skeleton << indent() << "int port = 9090;" << endl
             << indent() << "::apache::thrift::stdcxx::shared_ptr<" << svcname
             << "AsyncHandler> handler(new " << svcname << "AsyncHandler());" << endl
             << indent() << "::apache::thrift::stdcxx::shared_ptr<" << svcname
             << "AsyncProcessor> processor(new " << svcname << "AsyncProcessor(handler));" << endl
             << indent()
             << "::apache::thrift::stdcxx::shared_ptr<TProtocolFactory> protocolFactory(new "
                "TBinaryProtocolFactory());"
             << endl
             << indent()
             << "::apache::thrift::stdcxx::shared_ptr<TAsyncProtocolProcessor> "
                "protocolProcessor(new TAsyncProtocolProcessor(processor, protocolFactory));"
             << endl
             << endl
             << indent() << "TEvhttpServer server(protocolProcessor, port);" << endl
 ...
 */
void t_gen_cpp_generator::generate_service_multiface(t_service* tservice) {
 ...
 */
void t_gen_cpp_generator::generate_service_client(t_service* tservice, string style) {
  string ifstyle;
  if (style == "Cob") {
    ifstyle = "CobCl";
  }
  ...
  if (style != "Cob") {
    ...
    if (style == "Concurrent") {
      f_header_ << ", "
                   "::apache::thrift::stdcxx::shared_ptr<::apache::thrift::async::"
    ...
    if (extends.empty()) {
      ...
      f_header_ << ":" << indent() << "  " << extends << style << client_suffix << "(iprot, oprot";
      if (style == "Concurrent") {
        f_header_ << ", sync";
      }
      f_header_ << ") {}" << endl;
  ...
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    ...
    if (true) {
      ...
      if (style == "Concurrent") {
        out << indent()
            << "::apache::thrift::async::TConcurrentSendSentry sentry(this->sync_.get());" << endl;
      }
      if (style == "Cob") {
...
class ProcessorGenerator_gen {
public:
  ProcessorGenerator_gen(t_gen_cpp_generator* generator, t_service* service, const string& style);
...
protected:
  ...
  t_gen_cpp_generator* generator_;
  t_service* service_;
...
ProcessorGenerator_gen::ProcessorGenerator_gen(t_gen_cpp_generator* generator,
                                               t_service* service,
                                               const string& style)
...
void ProcessorGenerator_gen::generate_class_definition() {
  // Generate the dispatch methods
  ...
  if (generator_->gen_templates_) {
    f_header_ << indent() << "typedef void (" << class_name_ << "::*"
              ...
              << indent() << "  ProcessFunctions() : generic(nullptr), specialized(nullptr) "
              << "{}" << endl
              << indent() << "};" << endl
...
void ProcessorGenerator_gen::generate_dispatch_call(bool template_protocol) {
...
void ProcessorGenerator_gen::generate_process_functions() {
  vector<t_function*> functions = service_->get_functions();
  vector<t_function*>::iterator f_iter;
  for (f_iter = functions.begin(); f_iter != functions.end(); ++f_iter) {
    if (generator_->gen_templates_) {
...
void ProcessorGenerator_gen::generate_factory() {
 ...
 */
void t_gen_cpp_generator::generate_service_processor(t_service* tservice, string style) {
  ProcessorGenerator_gen generator(this, tservice, style);
  generator.run();
}
 ...
 */
void t_gen_cpp_generator::generate_function_helpers(t_service* tservice, t_function* tfunction) {
 ...
 */
void t_gen_cpp_generator::generate_process_function(t_service* tservice,
                                                    t_function* tfunction,
                                                    string style,
                                                    bool specialized) {
  ...
  if (style != "Cob") {
    ...
    out << indent() << "void* ctx = nullptr;" << endl
        << indent() << "if (this->eventHandler_.get() != nullptr) {" << endl
    ...
    indent_up();
    out << indent() << "if (this->eventHandler_.get() != nullptr) {" << endl
 ...
 * @param tservice The service to generate a server for.
 */
void t_gen_cpp_generator::generate_service_skeleton(t_service* tservice) {
  string svcname = tservice->get_name();
 ...
 */
void t_gen_cpp_generator::generate_deserialize_field(ostream& out,
                                                     t_field* tfield,
                                                     string prefix,
                                                     string suffix) {
 ...
 */
void t_gen_cpp_generator::generate_deserialize_struct(ostream& out,
                                                      t_struct* tstruct,
                                                      string prefix,
                                                      bool pointer) {
  if (pointer) {
...
void t_gen_cpp_generator::generate_deserialize_container(ostream& out,
                                                         t_type* ttype,
                                                         string prefix) {
  scope_up(out);
 ...
 */
void t_gen_cpp_generator::generate_deserialize_map_element(ostream& out,
                                                           t_map* tmap,
                                                           string prefix) {
  string key = tmp("_key");
  string val = tmp("_val");
...
void t_gen_cpp_generator::generate_deserialize_set_element(ostream& out,
                                                           t_set* tset,
                                                           string prefix) {
...
void t_gen_cpp_generator::generate_deserialize_list_element(ostream& out,
                                                            t_list* tlist,
                                                            string prefix,
                                                            bool use_push,
                                                            string index) {
 ...
 */
void t_gen_cpp_generator::generate_serialize_field(ostream& out,
                                                   t_field* tfield,
                                                   string prefix,
                                                   string suffix) {
  t_type* type = get_true_type(tfield->get_type());
 ...
 */
void t_gen_cpp_generator::generate_serialize_struct(ostream& out,
                                                    t_struct* tstruct,
                                                    string prefix,
                                                    bool pointer) {
  if (pointer) {
...
void t_gen_cpp_generator::generate_serialize_container(ostream& out, t_type* ttype, string prefix) {
  scope_up(out);
 ...
 *
 */
void t_gen_cpp_generator::generate_serialize_map_element(ostream& out, t_map* tmap, string iter) {
 ...
 */
void t_gen_cpp_generator::generate_serialize_set_element(ostream& out, t_set* tset, string iter) {
 ...
 */
void t_gen_cpp_generator::generate_serialize_list_element(ostream& out,
                                                          t_list* tlist,
                                                          string iter) {
 ...
 * @return Namespaces
 */
string t_gen_cpp_generator::namespace_prefix(string ns) {
 ...
 * @return Namespaces
 */
string t_gen_cpp_generator::namespace_open(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "";
 ...
 * @return Namespaces
 */
string t_gen_cpp_generator::namespace_close(string ns) {
  if (ns.size() == 0) {
    return "";
  }
  string result = "}";
 ...
 */
string t_gen_cpp_generator::type_name(t_type* ttype, bool in_typedef, bool arg) {
 ...
 */
string t_gen_cpp_generator::base_type_name(t_base_type::t_base tbase) {
  switch (tbase) {
  case t_base_type::TYPE_VOID:
    return "void";
  case t_base_type::TYPE_STRING:
 ...
 */
string t_gen_cpp_generator::declare_field(t_field* tfield,
                                          bool init,
                                          bool pointer,
                                          bool constant,
                                          bool reference) {
 ...
 */
string t_gen_cpp_generator::function_signature(t_function* tfunction,
                                               string style,
                                               string prefix,
                                               bool name_params) {
 ...
 */
string t_gen_cpp_generator::argument_list(t_struct* tstruct, bool name_params, bool start_comma) {
  string result = "";
 ...
 */
string t_gen_cpp_generator::type_to_enum(t_type* type) {
  type = get_true_type(type);
...
string t_gen_cpp_generator::get_include_prefix(const t_program& program) const {
...
string t_gen_cpp_generator::get_legal_program_name(std::string program_name) {
  std::size_t found = 0;
  while (true) {
